...
class Artiste
{
    ...
    #[Assert\Length(min: 2, max: 180, minMessage: "Le nom d'artiste doit contenir au moins {{ limit }} caractères", maxMessage: "Le nom d'artiste ne peut pas dépasser {{ limit }} caractères")]
    private ?string $displayName = null;

    #[ORM\Column(type: 'text', nullable: true)]
    #[Assert\Length(min: 10, max: 2000, minMessage: "La biographie doit contenir au moins {{ limit }} caractères", maxMessage: "La biographie ne peut pas dépasser {{ limit }} caractères")]
    private ?string $biography = null;

    #[ORM\Column(length: 120, nullable: true)]
    #[Assert\Length(max: 120, maxMessage: "La spécialité ne peut pas dépasser {{ limit }} caractères")]
    ...
    #[Assert\Regex(pattern: '/^https?:\/\/.+/', message: "L'URL doit commencer par http:// ou https://")]
    #[Assert\Length(max: 255, maxMessage: "L'URL du site ne peut pas dépasser {{ limit }} caractères")]
    ...
    #[Assert\File(maxSize: '5M', mimeTypes: ['image/jpeg', 'image/png', 'image/webp'], mimeTypesMessage: 'Veuillez télécharger une image valide (JPEG, PNG, WebP).')]
    ...
    #[Assert\Regex(pattern: '/^https?:\/\/.+/', message: "L'URL Facebook doit commencer par http:// ou https://")]
    private ?string $facebook = null;

    #[Assert\Url(message: "Veuillez saisir une URL valide pour Instagram")]
    #[Assert\Regex(pattern: '/^https?:\/\/.+/', message: "L'URL Instagram doit commencer par http:// ou https://")]
    private ?string $instagram = null;

    #[Assert\Url(message: "Veuillez saisir une URL valide pour Behance")]
    #[Assert\Regex(pattern: '/^https?:\/\/.+/', message: "L'URL Behance doit commencer par http:// ou https://")]
    ...
    public function setSocialLinks(?array $socialLinks): static { $this->socialLinks = $socialLinks; return $this; }
